...
@Api(tags = "账管人管理")
@RestController
@RequestMapping("/org/zguan")
public class OrgZguanController extends BaseController
{
    @Autowired
     ...
     * 查询部门列表
     */
    @ApiOperation(value = "账管人审核")
    @PreAuthorize("@ss.hasPermi('zguanorg:check:index')")
    @GetMapping("/check")
     ...
     * 查询部门列表
     */
    @PreAuthorize("@ss.hasPermi('zguanorg:info:list')")
    @GetMapping("/list")
    public TableDataInfo list(TOrgInfo tOrgInfo)
    {
        startPage();
        tOrgInfo.setDeptType("04");
     ...
     * 导出部门列表
     */
    @PreAuthorize("@ss.hasPermi('zguanorg:info:export')")
     ...
     * 获取部门详细信息
     */
    @PreAuthorize("@ss.hasPermi('zguanorg:info:query')")
     ...
     * 新增部门
     */
    @PreAuthorize("@ss.hasPermi('zguanorg:info:add')")
     ...
     * 修改部门
     */
    @PreAuthorize("@ss.hasPermi('zguanorg:info:edit')")
     ...
     * 删除部门
     */
    @PreAuthorize("@ss.hasPermi('zguanorg:info:remove')")
