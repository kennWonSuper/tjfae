...
@Api(tags = "投管人管理")
@RestController
@RequestMapping("/org/tguan")
public class OrgTguanController extends BaseController
{
    @Autowired
     ...
     * 查询部门列表
     */
    @ApiOperation(value = "投管人审核")
    @PreAuthorize("@ss.hasPermi('tguanorg:check:index')")
    @GetMapping("/check")
     ...
     * 查询部门列表
     */
    @PreAuthorize("@ss.hasPermi('tguanorg:info:list')")
    @GetMapping("/list")
    public TableDataInfo list(TOrgInfo tOrgInfo)
    {
        startPage();
        tOrgInfo.setDeptType("06");
     ...
     * 导出部门列表
     */
    @PreAuthorize("@ss.hasPermi('tguanorg:info:export')")
     ...
     * 获取部门详细信息
     */
    @PreAuthorize("@ss.hasPermi('tguanorg:info:query')")
     ...
     * 新增部门
     */
    @PreAuthorize("@ss.hasPermi('tguanorg:info:add')")
     ...
     * 修改部门
     */
    @PreAuthorize("@ss.hasPermi('tguanorg:info:edit')")
     ...
     * 删除部门
     */
    @PreAuthorize("@ss.hasPermi('tguanorg:info:remove')")
