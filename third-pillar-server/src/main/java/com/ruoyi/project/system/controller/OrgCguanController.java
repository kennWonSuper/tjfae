...
@Api(tags = "存管人管理")
@RestController
@RequestMapping("/org/cguan")
public class OrgCguanController extends BaseController
{
    @Autowired
     ...
     * 查询部门列表
     */
    @ApiOperation(value = "存管人审核")
    @PreAuthorize("@ss.hasPermi('cguanorg:check:index')")
    @GetMapping("/check")
     ...
     * 查询部门列表
     */
    @PreAuthorize("@ss.hasPermi('cguanorg:info:list')")
    @GetMapping("/list")
    public TableDataInfo list(TOrgInfo tOrgInfo)
    {
        startPage();
        tOrgInfo.setDeptType("05");
     ...
     * 导出部门列表
     */
    @PreAuthorize("@ss.hasPermi('cguanorg:info:export')")
     ...
     * 获取部门详细信息
     */
    @PreAuthorize("@ss.hasPermi('cguanorg:info:query')")
     ...
     * 新增部门
     */
    @PreAuthorize("@ss.hasPermi('cguanorg:info:add')")
     ...
     * 修改部门
     */
    @PreAuthorize("@ss.hasPermi('cguanorg:info:edit')")
     ...
     * 删除部门
     */
    @PreAuthorize("@ss.hasPermi('cguanorg:info:remove')")
